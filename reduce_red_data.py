...
calib_prefix = os.path.join(out_dir,'wifesR')  # Removed date from prefix
...
proc_steps = [
    ...
    {'step':'cube_gen'       , 'run':True, 'suffix':'08',
     'args':{'multithread':multithread,
             'adr':True,
             # 'dw_set':0.44,
             'wmin_set':5330.0,   # Hard wavelength limits for R3000
             'wmax_set':9800.0,   # may need tweaking
	    }},
    #------------------
    ...
    {'step':'save_3dcube'    , 'run':True, 'suffix':'11', 'args':{}}  
     # This step does not work without standard stars
     # See section RUN THE PROCESSING STEPS
    #------------------
    ]
...
for step in proc_steps:
    ...
    if step_run:
    	#if step_name == 'save_3dcube':  # Uncomment if no std stars
    	#    prev_suffix = '08'  # Can't do 09, 10 without standard stars 
        func(obs_metadata,
